...
- `configurations/homelab/configuration.nix` is the main module for host/system settings; it also imports the shared `configurations/common/crypto.nix` and `configurations/common/synology.nix` modules for a CIFS mount at `//synology.local/Library1` with credentials rendered by sops-nix.
...
- `configurations/thinkpad/configuration.nix` contains the desktop configuration; it imports the shared `configurations/common/crypto.nix` and `configurations/common/synology.nix` modules for a CIFS mount at `//synology.local/Library1` with credentials rendered by sops-nix.
...
- `/etc/nixos/namecheap.env` – Namecheap API credentials used by ACME via `security.acme.certs."<baseDomain>".environmentFile`.
...
The Synology CIFS credentials (shared by homelab and thinkpad via `configurations/common/synology.nix`) are stored encrypted in `secrets/common.yaml` and rendered at activation time by sops-nix using `/etc/ssh/ssh_host_ed25519_key` as an Age identity (set up by `configurations/common/crypto.nix`). The host key is generated automatically when absent.
