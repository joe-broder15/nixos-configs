...
The Synology CIFS credentials (shared by homelab and thinkpad via `configurations/common/synology.nix`) and the Namecheap API credentials (used by `configurations/homelab/proxy.nix` for ACME DNS challenges) are stored encrypted in `secrets/common.yaml` and rendered at activation time by sops-nix using `/etc/ssh/ssh_host_ed25519_key` as an Age identity (set up by `configurations/common/crypto.nix`). The host key is generated automatically when absent.
