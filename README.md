...
- `configurations/thinkpad/configuration.nix` contains the desktop configuration, including a CIFS mount for `//synology.local/Library1` with credentials rendered by sops-nix.
- `configurations/thinkpad/hosts.nix` adds static `networking.hosts` entries for `synology.local`, `proxmox.local`, and `homelab.local`.
- Hardware configuration is read from `/etc/nixos/hardware-configuration.nix` on the host (not versioned in this repo).
...
The ThinkPad's CIFS credentials are stored encrypted in `secrets/common.yaml` and rendered at activation time by sops-nix using `/etc/ssh/ssh_host_ed25519_key` as an Age identity. The host key is generated automatically when absent.
